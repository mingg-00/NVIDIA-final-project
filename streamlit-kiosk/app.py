...
def main():
    ...
    with header_col3:
        ...
        with col_mode:
            if st.button(f"👥 {'일반모드' if st.session_state.elderly_mode else '어르신모드'}", key="mode_toggle"):
...
def show_order_type_selection():
    ...
    with col2:
        ...
        if st.button("🍽️ 매장에서 식사", key="dine_in", use_container_width=True):
    ...
    with st.container():
        ...
        with col2:
            ...
            if item["name"] in diet_lists["vegan"]:
                ...
                tags_html += '<span class="diet-tag normal-tag">일반</span>'
            
            if item.get("is_customizable"):
                tags_html += '<span class="diet-tag customizable-tag">커스터마이징 가능</span>'
            ...
            tags_html += f'<span class="diet-tag normal-tag">나트륨 {item["sodium"]}mg</span>'
            
            st.markdown(tags_html, unsafe_allow_html=True)
            
            button_col1, button_col2 = st.columns(2)
            with button_col1:
                if st.button(f"담기", key=f"add_large_{item['id']}", use_container_width=True):
                    add_to_cart(item)
            ...
            with button_col2:
                if st.button(f"상세", key=f"detail_large_{item['id']}", use_container_width=True):
...
def show_payment_view():
    ...
    with payment_col1:
        if st.button("💳 신용카드", use_container_width=True):
            process_payment("card")
        
        if st.button("📱 모바일 결제", use_container_width=True):
            process_payment("mobile")
    
    with payment_col2:
        if st.button("💵 현금", use_container_width=True):
            process_payment("cash")
        
        if st.button("🎁 상품권", use_container_width=True):
...
def show_processing_view():
    ...
    if st.session_state.selected_payment_method:
        st.write(f"결제 방법: {method_names.get(st.session_state.selected_payment_method, st.session_state.selected_payment_method)}")
...
def show_completed_view():
    ...
    st.markdown("5초 후 자동으로 처음 화면으로 돌아갑니다.")
    st.markdown("</div>", unsafe_allow_html=True)
